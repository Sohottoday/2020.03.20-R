...
행렬 matrix
인덱싱 : [행의 인덱싱, 열의 인덱싱, drop]           drop=2차원 구조를 유지할지 안할지, 안할경우 자동으로 벡터로 변환시킴
matrix(data=벡터, nrow=행의 갯수, ncol=열의 갯수)         행열중에 하나만 줘도 알아서 계산해서 생성
matrix(data=벡터, nrow=행의 갯수, ncol=열의 갯수, byrow=TRUE)   byrow 를 생략하면 기본값 false, byrow는 열을 우선으로 채울지 행을 우선으로 채울지(defalt는 열 우선, true는 행 우선)
rbind(벡터들)       
cbind(벡터들)        보통 이미 만들어져있는 matrix에 행이나 열을 추가시키는 함수이지만 생성도 가능.
dim(m) = 행렬이 몇차원인지 체크하는 함수
nrow() = 몇행인지 알고싶을때 / ncol() = 몇열인지 알고싶을때
rowSums(), colSums() = 각각 행이나 열의 합
